...
export default function SimulationPanel() {
  ...
  return (
    /* Changed: Added h-full and flex flex-col to the outer container to ensure it fills the parent and manages children space */
    <div className="console-panel flex flex-col h-full w-full overflow-hidden bg-white border border-gray-300">
        ...
        <table className="w-full table-fixed">
          <thead>
            <tr className="text-gray-600">
              ...
              <th className="px-3 py-1 font-bold text-left text-[10px] w-[33%]">Message</th>
              <th className="px-4 py-1 font-bold text-left text-[10px] w-[10%]">Time</th>
              <th className="px-4 py-1 font-bold text-left text-[10px] w-[10%]">Date</th>
              <th className="px-4 py-1 font-bold text-left text-[10px] w-[15%]">Location</th>
      ...
      {/* Changed: overflow-y-scroll forces the scrollbar track to stay, keeping the UI stable. bg-white ensures it's never transparent. */}
      <div className="flex-1 bg-white overflow-y-scroll min-h-0 flex flex-col">
        {(isTableEmpty || isFilterEmpty) ? (
          ...
          /* Changed: flex-1 on this div inside the parent flex container ensures the white background stretches to the bottom */
                ...
                {filteredLogs.map((log, index) => (
                  <tr key={index} className="hover:bg-blue-50/50 h-9 transition-colors group border-b border-gray-100">
                    ...
                    <td className="px-4 py-1 text-center w-10">
                      <Trash2 
                        className="text-gray-400 cursor-pointer inline-block transition-colors hover:text-red-500" 
                        size={15} 
